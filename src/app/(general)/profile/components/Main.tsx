  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        localStorage.setItem("asiwajuMemberProfile", JSON.stringify(memberData));
      } else {
      ...
      {member && (
        <div className="w-full max-w-md mt-[3rem] flex flex-col items-center animate-fade-in-up">
              ...
              {!isFlipped ? (
                  ...
                  <div className="absolute top-16 left-1/2 -translate-x-1/2 z-20">
                    <div className="w-28 h-28 rounded-full border-[5px] border-white shadow-md overflow-hidden bg-gray-50 flex items-center justify-center">
                      ...
                      <img 
                        ...
                        onError={(e) => {
                          (e.target as HTMLImageElement).src = 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="100" height="100" viewBox="0 0 24 24" fill="%23ccc"><path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"/></svg>';
                        }}
                      />
                    </div>
                  </div>

                  <div className="flex-1 flex flex-col items-center justify-start pt-16 pb-5 px-6 text-center">
                    <h3 className="text-xl font-bold text-gray-900 mb-2 leading-tight line-clamp-2 uppercase">
                      {member.fullName}
                    </h3>
                    
                    {member.verified ? (
                      <div className="flex items-center gap-1.5 text-emerald-700 font-bold uppercase tracking-[0.1em] text-[10px] mb-4 bg-emerald-50 border border-emerald-100 px-3 py-1 rounded-full">
                      ...
                      <div className="flex items-center gap-1.5 text-amber-600 font-bold uppercase tracking-[0.1em] text-[10px] mb-4 bg-amber-50 border border-amber-100 px-3 py-1 rounded-full">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-3.5 h-3.5">
                    ...
                    <div className="w-full bg-gray-50 rounded-xl p-4 mb-auto border border-gray-100 shadow-inner">
                      <div className="flex flex-col mb-3">
                        <span className="text-[9px] uppercase tracking-widest text-gray-400 font-bold">Membership ID</span>
                        <span className="font-mono font-bold text-lg tracking-widest text-gray-800">{member.membershipId}</span>
                      </div>
                      <div className="flex flex-col">
                        <span className="text-[9px] uppercase tracking-widest text-gray-400 font-bold">Ward / District</span>
                        <span className="font-semibold text-sm text-gray-700">{member.ward}</span>
                      </div>
                    </div>
                    ...
                    <div className="w-full mt-4 flex flex-col items-center justify-center gap-2">
                      <div className="w-3/4 h-6 opacity-30 flex gap-[2px] justify-center items-center">
                        {[...Array(24)].map((_, i) => (
                          <div key={i} className="bg-gray-800 h-full" style={{ width: `${Math.max(1, Math.random() * 4)}px` }}></div>
                        ))}
                      </div>
                      <p className="text-[9px] text-gray-400 font-bold uppercase tracking-wider">
                        Issued: {new Date(member.createdAt).toLocaleDateString(undefined, { year: 'numeric', month: 'short', day: 'numeric' })}
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-col h-full w-full bg-white p-6 relative animate-fade-in text-gray-800">
                  <div className="w-full border-b-2 border-gray-200 pb-3 mb-5 mt-4 text-center">
                    ...
                    <p className="text-[10px] uppercase tracking-widest text-gray-500 font-bold mt-1">Official Property</p>
                  </div>
                  
                  <div className="flex-1 flex flex-col gap-3 text-xs leading-relaxed text-gray-600 px-1 text-justify">
                    <p>
                      <strong>1.</strong> This identity card is strictly non-transferable and remains the absolute property of the Asiwaju Boys organization.
                    </p>
                    <p>
                    ...
                    <p>
                      <strong>3.</strong> If this card is found, please drop it at the nearest police station or return it to the organization's national headquarters.
                    </p>
                  </div>

                  <div className="w-full mt-auto pt-6 pb-2 flex flex-col items-center border-t border-gray-100">
                    {/* Dummy Signature Area */}
                    <div className="font-[cursive] text-xl text-gray-800 mb-1 opacity-80" style={{ fontFamily: "'Brush Script MT', cursive, sans-serif" }}>
                      Dr. Freeman Ezeiriaku
                    </div>
                    <div className="w-40 h-[1px] bg-gray-400 mb-2"></div>
                    <span className="text-[9px] uppercase tracking-widest text-gray-500 font-bold">Authorized Signature</span>
                  </div>

                  {/* Aesthetic barcode at bottom back */}
                   <div className="w-full mt-4 flex justify-center h-8 opacity-40 gap-[2px]">
                        {[...Array(30)].map((_, i) => (
                          <div key={i} className="bg-gray-800 h-full" style={{ width: `${Math.max(1, Math.random() * 5)}px` }}></div>
                        ))}
                  </div>
                </div>
              )}
          ...
          <div className="mt-6 flex flex-col items-center justify-center gap-3 w-full max-w-[300px]">
            ...
            >
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className={`w-4 h-4 transition-transform duration-300 ${isFlipped ? 'rotate-180' : ''}`}>
